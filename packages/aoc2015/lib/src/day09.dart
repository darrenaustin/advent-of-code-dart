...
class Day09 extends AdventDay {
  ...
  @override
  dynamic part2(String input) => minDistanceBy(parseRoutes(input), numMaxComparator);
  ...
  int? minDistanceBy(
    ...
    int? shortestFrom(String location, Set<String> toVisit) {
      ...
      return toVisit.map((neighbor) {
        final int? destDistance = destinations[neighbor];
        if (destDistance != null) {
          final int? shortestFromDest = shortestFrom(neighbor, toVisit.difference({neighbor}));
