...
class Day21 extends AdventDay {
  ...
  String scramble(String password, String input) =>
    input
      .lines
      .fold(password, (pwd, op) => performOperation(pwd, op));

  String unscramble(String password, String input) =>
    input
      .lines
      .reversed
  ...
  String performOperation(String password, String operation) {
      final swapPosMatch = RegExp(r'swap position (\d+) with position (\d+)').firstMatch(operation);
      if (swapPosMatch != null) {
        return password.swapPositions(int.parse(swapPosMatch.group(1)!), int.parse(swapPosMatch.group(2)!));
      }
      final swapLetterMatch = RegExp(r'swap letter ([a-z]) with letter ([a-z])').firstMatch(operation);
      if (swapLetterMatch != null) {
        return password.swapLetters(swapLetterMatch.group(1)!, swapLetterMatch.group(2)!);
      }
      final rotateMatch = RegExp(r'rotate (left|right) (\d+) step').firstMatch(operation);
      if (rotateMatch != null) {
        return password.rotate((rotateMatch.group(1)! == 'left' ? -1 : 1) * int.parse(rotateMatch.group(2)!));
      }
      final rotateLetterMatch = RegExp(r'rotate based on position of letter ([a-z])').firstMatch(operation);
      if (rotateLetterMatch != null) {
        return password.rotateAroundLetter(rotateLetterMatch.group(1)!);
      }
      final reveseMatch = RegExp(r'reverse positions (\d+) through (\d+)').firstMatch(operation);
      if (reveseMatch != null) {
        return password.reversePositions(int.parse(reveseMatch.group(1)!), int.parse(reveseMatch.group(2)!));
      }
      final moveMatch = RegExp(r'move position (\d+) to position (\d+)').firstMatch(operation);
      if (moveMatch != null) {
        return password.movePosition(int.parse(moveMatch.group(1)!), int.parse(moveMatch.group(2)!));
      }
  ...
  String performReverseOperation(String password, String operation) {
      final swapPosMatch = RegExp(r'swap position (\d+) with position (\d+)').firstMatch(operation);
      if (swapPosMatch != null) {
        return password.swapPositions(int.parse(swapPosMatch.group(2)!), int.parse(swapPosMatch.group(1)!));
      }
      final swapLetterMatch = RegExp(r'swap letter ([a-z]) with letter ([a-z])').firstMatch(operation);
      if (swapLetterMatch != null) {
        return password.swapLetters(swapLetterMatch.group(2)!, swapLetterMatch.group(1)!);
      }
      final rotateMatch = RegExp(r'rotate (left|right) (\d+) step').firstMatch(operation);
      if (rotateMatch != null) {
        return password.rotate((rotateMatch.group(1)! == 'left' ? 1 : -1) * int.parse(rotateMatch.group(2)!));
      ...
      final moveMatch = RegExp(r'move position (\d+) to position (\d+)').firstMatch(operation);
      if (moveMatch != null) {
        return password.movePosition(int.parse(moveMatch.group(2)!), int.parse(moveMatch.group(1)!));
      }
      throw Exception('Unable to parse operation: $operation');
  }
}
