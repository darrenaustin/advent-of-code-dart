        ...
        case OpCode.tgl:
          ...
          if (addr < instructions.length) {
            ...
            switch (targetInstruction.opCode) {
              case OpCode.cpy:
                toggledInstruction = Instruction(OpCode.jnz, targetInstruction.op1, targetInstruction.op2);
                break;
              case OpCode.inc:
                toggledInstruction = Instruction(OpCode.dec, targetInstruction.op1);
                break;
              case OpCode.dec:
                toggledInstruction = Instruction(OpCode.inc, targetInstruction.op1);
                break;
              case OpCode.jnz:
                toggledInstruction = Instruction(OpCode.cpy, targetInstruction.op1, targetInstruction.op2);
                break;
              case OpCode.tgl:
                toggledInstruction = Instruction(OpCode.inc, targetInstruction.op1);
                break;
...
class Instruction {
  ...
  static final _opCode = { 'cpy': OpCode.cpy, 'inc': OpCode.inc, 'dec': OpCode.dec, 'jnz': OpCode.jnz, 'tgl': OpCode.tgl };
  static final _pattern = RegExp('(${_opCode.keys.join('|')})' r' ((-?\d+)|[a-d])( ((-?\d+)|[a-d]))?');
  ...
  @override
  String toString() => '${opCode.name} ${op1.toString()} ${op2?.toString() ?? ''}';
}
