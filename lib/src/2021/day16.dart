...
class Day16 extends AdventDay {
  Day16() : super(2021, 16, solution1: 984, solution2: 1015320896946);
  ...
  dynamic part1() {
    ...
    return packetVersionSum(Packet.parse(BitStream(inputData())));
  }

  @override
  dynamic part2() {
    return Packet.parse(BitStream(inputData())).eval();
  }
}
...
class BitStream {
  ...
  BitStream(String hexDigits) : _bits = [] {
    for (final d in hexDigits.split('')) {
      ...
      _bits.addAll(value.toRadixString(2).padLeft(4, '0').split('').map((b) => b == '1' ? 1 : 0));
    }
  }
